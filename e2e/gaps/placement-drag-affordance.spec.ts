...
test.describe('배치 페이지에서 식물 드래그가 실제로 작동해야 한다', () => {
  test('식물 핀을 드래그하면 위치가 변경되어야 한다', async ({ page }) => {
    ...
    // Find the first plant pin (🌿 inside the placement canvas)
    ...
    // Attempt drag using mouse API (plantPin has no drag handler)
    ...
    // Small wait for any potential state update
    ...
    // GAP ASSERTION: position should have changed after drag
    // Current code has no drag handlers, so position stays the same
    const moved =
